...
const { dockerComposePull, dockerComposeUp, dockerComposeDown } = require('./docker');
...
async function routineStop(emitter) {
  if (!acquireLock(emitter, 'stop')) return;
  try {
    emitter.emit('status', { status: 'stopping' });
    await dockerComposeDown(emitter);
    emitter.emit('status', { status: 'stopped' });
    console.log('[BEDROCK] Production stack stopped.');
  } catch (err) {
    console.error('[BEDROCK] stop error:', err);
    emitter.emit('status', { status: 'error', reason: `Stop failed: ${err.message}` });
  } finally {
    releaseLock('stop');
  }
}
