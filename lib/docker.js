...
function dockerComposePull(tag, emitter) {
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn('docker', ['compose', '-f', CONFIG.yamlName, 'pull'], {
      cwd: CONFIG.repoPath,
      env: process.env,
    });
...
// WARNING! THIS FUNCTION REMOVES REDIS, WHICH SHOULD NEVER BE REMOVED

// function removeAllContainers(emitter) {
//   return new Promise((resolve, reject) => {
//     const list = spawn('docker', ['ps', '-aq'], { env: process.env });

//     let ids = '';
//     list.stdout.on('data', (chunk) => { ids += chunk.toString(); });

//     list.on('error', (err) => reject(new Error(`Failed to list containers: ${err.message}`)));

//     list.on('close', (code) => {
//       if (code !== 0) return reject(new Error(`docker ps -aq exited with code ${code}`));

//       const containerIds = ids.trim().split('\n').filter(id => id.trim());
//       if (containerIds.length === 0) {
//         console.log('[BEDROCK] removeAllContainers: no containers found, skipping.');
//         return resolve();
//       }

//       console.log(`[BEDROCK] removeAllContainers: removing ${containerIds.length} container(s).`);
//       emitter.emit('status', { status: 'soft-restarting', progress: `Removing ${containerIds.length} existing container(s)...` });

//       const rm = spawn('docker', ['rm', '-f', ...containerIds], { env: process.env });
//       let stderr = '';

//       rm.stderr.on('data', (chunk) => { stderr += chunk.toString(); });

//       rm.on('error', (err) => reject(new Error(`Failed to remove containers: ${err.message}`)));

//       rm.on('close', (rmCode) => {
//         if (rmCode === 0) {
//           console.log('[BEDROCK] removeAllContainers: all containers removed.');
//           resolve();
//         } else {
//           reject(new Error(stderr.trim() || `docker rm -f exited with code ${rmCode}`));
//         }
//       });
//     });
//   });
...
async function dockerComposeUp(emitter) {
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn('docker', ['compose', '-f', CONFIG.yamlName, 'up', '-d', '--remove-orphans'], {
      cwd: CONFIG.repoPath,
      env: process.env,
    });
...
function dockerComposeDown(emitter) {
  return new Promise((resolve, reject) => {
    if (!fs.existsSync(CONFIG.yamlPath)) {
      return reject(new Error(`YAML file not found at "${CONFIG.yamlPath}". Check YAML_NAME in .env.`));
    }

    emitter.emit('status', { status: 'stopping', progress: 'Running docker compose down...' });
    console.log(`[BEDROCK] docker compose down -f ${CONFIG.yamlPath}`);

    const proc = spawn('docker', ['compose', '-f', CONFIG.yamlName, 'down'], {
      cwd: CONFIG.repoPath,
      env: process.env,
    });
    ...
    proc.stdout.on('data', (chunk) => {
      for (const line of chunk.toString().split('\n').filter(l => l.trim())) {
        emitter.emit('status', { status: 'stopping', progress: line });
      }
    });
    ...
    proc.on('error', (err) => reject(new Error(`Failed to spawn docker compose down: ${err.message}`)));

    proc.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(stderr.trim() || `docker compose down exited with code ${code}`));
    });
  });
}

module.exports = { dockerComposePull, dockerComposeUp, dockerComposeDown };
